...
bool parse_options(int argc, char * argv[])
{
  ...
  desc.add_options()
	...
	("attribute,a",po::value(&options.attribute),
	 "shapefile attribute name")
  ...
  if(options.width<0 && options.height>0)
	{
	  options.width = static_cast<int>(options.height * (options.lon2-options.lon1)/(options.lat2-options.lat1));

	}

  if(options.height<0 && options.width>0)
	{
	  options.height = static_cast<int>(options.width * (options.lat2-options.lat1)/(options.lon2-options.lon1));
	}
...
map<string,int> make_attribute_map(const set<string> & theSet)
{
  map<string,int> ret;
  int i = 1;
  for(set<string>::const_iterator it = theSet.begin();
	  it != theSet.end();
	  ++it)
	{
  ...
  if(options.verbose)
	{
	  cout << "Rendering "
		   ...
		   << "x"
		   << theImage.Height()
		   << " size image, this may take a while"
		   << endl;
	}
					  ...
					  const map<string,int> & theMap)
{
  ...
  for(map<string,int>::const_iterator it = theMap.begin();
	  it != theMap.end();
	  ++it)
	{
	  out << it->first << '\n';
