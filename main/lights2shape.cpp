...
void read_lights() {
  ...
  const int j1 =
      static_cast<int>(std::floor((maxlatitude - globals.y2) * ratio));
  const int j2 =
      static_cast<int>(std::ceil((maxlatitude - globals.y1) * ratio));
...
void create_shape() {
  ...
  NFmiEsriAttributeName *attribute =
      new NFmiEsriAttributeName("INTENSITY", kFmiEsriInteger, 4, 0);
  shape.Add(attribute);

  for (set<int>::const_iterator it = globals.levels.begin();
       it != globals.levels.end(); ++it) {
    if (globals.verbose)
