...
bool parse_options(int argc, char *argv[])
{
  ...
  desc.add_options()("help,h", "print out help message")(
      ...
      "attributes,a",
      po::value(&options.attributes),
      "shapefile attributes to be printed (comma separated)")(
      "unique,u",
      po::value(&options.uniqueattribute),
      "force a specific attribute to have unique values")(
      "shapefile,s", po::value(&options.shapefile), "shapefile (without suffix)")(
      "condition,c", po::value(&options.condition), "search condition (for example CLASS>2)")(
      "lat,y", po::value(&options.latitude), "latitude of the searched coordinate")(
      "lon,x", po::value(&options.longitude), "longitude of the searched coordinate")(
      "coordinatefile,l",
      ...
      "radius,r", po::value(&options.searchradius), "maximum search radius")(
      "maxcount,n", po::value(&options.maxcount), "maximum number of search results")(
      "delimiter,d", po::value(&options.delimiter), "delimiter string for output columns")(
      "projection,p", po::value(&options.projection), "projection definition (default=latlon)");
...
float latlon_point_distance(double theX1, double theY1, double theX2, double theY2)
{
...
float world_point_distance(double theX1, double theY1, double theX2, double theY2)
{
...
void isosegment_center(
    double theX1, double theY1, double theX2, double theY2, double &theX, double &theY)
{
  ...
  theY = atan2(sin(lat1) + sin(lat2), sqrt((cos(lat1) + Bx) * (cos(lat1) + Bx) + By * By));
}
...
float world_line_distance(
    double theX, double theY, double theX1, double theY1, double theX2, double theY2)
{
  float m = NFmiGeoTools::DistanceFromLineSegment(theX, theY, theX1, theY1, theX2, theY2);
  return m / 1000;
}
...
NFmiEsriAttributeType establish_attribute_type(const NFmiEsriShape &theShape, const string &theName)
{
  const Imagine::NFmiEsriShape::attributes_type &attributes = theShape.Attributes();
...
void filter_out_duplicates(const NFmiEsriShape &theShape, multimap<float, int> &theData)
{
  ...
  for (multimap<float, int>::const_iterator it = theData.begin(); it != theData.end(); ++it)
  {
    int pos = it->second;
                         ...
                         const std::string &theName = "")
{
  ...
  for (NFmiEsriShape::elements_type::size_type i = 0; i < elements.size(); i++)
  {
    ...
    float dist;
    if (options.projection == "latlon")
      dist = latlon_point_distance(elem->X(), elem->Y(), theLatLon.X(), theLatLon.Y());
    else
      dist = world_point_distance(elem->X(), elem->Y(), worldxy.X(), worldxy.Y());
                        ...
                        const std::string &theName = "")
{
  ...
  for (NFmiEsriShape::elements_type::size_type i = 0; i < elements.size(); i++)
  {
    ...
    const NFmiEsriPolyLine *elem = static_cast<const NFmiEsriPolyLine *>(elements[i]);
...
int domain(int argc, char *argv[])
{
  ...
  else
  {
    ...
    for (LocationList::const_iterator it = places.begin(); it != places.end(); ++it)
    {
