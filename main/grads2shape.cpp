...
int domain(int argc, const char *argv[])
{
  ...
  while (in >> noskipws >> record_type >> record_level >> record_points)
  {
    if (record_type == 1)
    {
      Imagine::NFmiEsriPolyLine *line = new Imagine::NFmiEsriPolyLine(counter);
    ...
    else if (record_type == 2)
    {
      ...
      cout << "Skipping skip record with bbox: " << lon1 << ',' << lat1 << " ... " << lon2 << ','
           << lat2 << " and length " << length << endl;
    }
    else
...
int main(int argc, const char *argv[])
{
  ...
  {
    cerr << "Error: grads2shape failed due to" << endl << "--> " << e.what() << endl;
  }
  catch (...)
  {
