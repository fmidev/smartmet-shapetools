...
int domain(int argc, const char *argv[])
{
  ...
  while (in >> noskipws >> record_type >> record_level >> record_points)
  {
    ...
    if (record_type == 1)
    {
      cout << "# Record level = " << static_cast<int>(record_level) << endl
           << "# Record size = " << static_cast<int>(record_points) << endl;
    ...
    {
      cout << "# Record start level = " << static_cast<int>(record_level) << endl
           << "# Record end level = " << static_cast<int>(record_points) << endl;
      ...
      throw runtime_error("Record type " + NFmiStringTools::Convert(static_cast<int>(record_type)) +
                          " is unknown");
  }
...
int main(int argc, const char *argv[])
{
  ...
  {
    cerr << "Error: gradsdump failed due to" << endl << "--> " << e.what() << endl;
  }
  catch (...)
  {
