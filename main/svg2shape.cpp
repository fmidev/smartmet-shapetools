...
bool parse_options(int argc, char *argv[]) {
  ...
  desc.add_options()("help,h", "print out help message")(
      ...
      "infiles", po::value<std::vector<std::string>>(&options.infiles),
      "input files")("shape,o", po::value(&options.shapename),
                     "shapefile name ('out')")(
      "field,f", po::value(&options.shapename),
      "field name for the paths ('NAME')");
  ...
  if (opt.count("version") != 0) {
    std::cout << "svg2shape v1.0 (" << __DATE__ << ' ' << __TIME__ << ')'
              << std::endl;
  }

  if (opt.count("help")) {
    std::cout
        << "Usage: svg2shape [options] [infiles]" << std::endl
        << std::endl
        << "svg2shape converts SVG paths generated by shape2svg back to a shape"
        << std::endl
        << std::endl
        << desc << std::endl;
    return false;
...
void make_shape(const std::map<std::string, NFmiSvgPath> &paths) {
  ...
  NFmiEsriAttributeName *attribute = new NFmiEsriAttributeName(
      options.fieldname, std::string(), max_name_length(paths));
  shape.Add(attribute);
