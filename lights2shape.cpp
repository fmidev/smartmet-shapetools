  ...
  if(cmdline.isOption('b'))
	{
	  ...
	  if(globals.x1<-180 || globals.x1>180 ||
		 globals.x2<-180 || globals.x2>180 ||
		 globals.y1<-90 || globals.y1>90 ||
...
void read_lights()
{
  ...
  if(globals.verbose)
	cout << "The grid to be extracted is "
		 << i2-i1+1 << 'x'
		 << j2-j1+1 << '+'
		 << i1 << '+'
		 << j1 << endl;
...
void create_shape()
{
  ...
  NFmiEsriAttributeName * attribute = new NFmiEsriAttributeName("INTENSITY",
																kFmiEsriInteger,
																4,0);
  shape.Add(attribute);
  
  for(set<int>::const_iterator it = globals.levels.begin();
	  it != globals.levels.end();
	  ++it)
	{
	  if(globals.verbose)
